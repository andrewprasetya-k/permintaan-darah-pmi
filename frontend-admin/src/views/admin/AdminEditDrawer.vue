...
watch(
  () => props.isOpen,
  (newVal) => {
    if (newVal && adminStore.selectedAdmin) {
      formData.value = {
        adminUsername: adminStore.selectedAdmin.adminUsername,
        adminPassword: '',
        adminName: adminStore.selectedAdmin.adminName,
        adminEmail: adminStore.selectedAdmin.adminEmail,
...
<template>
  <Teleport to="body">
    ...
    <AppModal
      ...
      title="Simpan Perubahan Admin"
      :description="`Perubahan data admin ${formData.adminName || formData.adminUsername || ''} akan disimpan.`"
