...
const handleSubmit = async () => {
  ...
  try {
    ...
    flag.value = {
      variant: 'success',
      title: 'Komponen dibuat',
      message: `${formData.value.komponenDarah} berhasil ditambahkan ke sistem.`,
    }
    emit('submit')
    resetForm()
    emit('close')
    ...
    flag.value = {
      variant: 'error',
      title: 'Operasi gagal',
      message: error instanceof Error ? error.message : 'Gagal membuat komponen darah baru.',
    }
  } finally {
...
<template>
  <Teleport to="body">
    ...
    <Transition name="drawer">
      ...
      >
        ...
        <div class="flex items-center justify-between px-10 py-8 border-b border-gray-200">
          <div>
            <h2 class="text-xl font-semibold text-gray-900">Tambah Komponen Darah</h2>
            <p class="text-sm text-gray-400 mt-0.5">Tambahkan komponen darah baru ke sistem</p>
          </div>
          <button
            @click="handleClose"
    ...
    <AppModal
      ...
      title="Simpan Komponen Darah Baru"
      :description="`Komponen ${formData.komponenDarah || 'baru'} akan ditambahkan ke sistem.`"
